...
sign_action (GtkWidget *widget, const char *action_name, GVariant *param)
{
    ...
    if (!GPG_IS_OK (err)) {
        if (gpgme_err_code (err) == GPG_ERR_EALREADY) {
            ...
            w = adw_message_dialog_new (GTK_WINDOW (self), NULL, NULL);
            adw_message_dialog_format_body (ADW_MESSAGE_DIALOG (w),
                                            _("This key was already signed by\n“%s”"),
                                            label);
            g_signal_connect (w, "response", G_CALLBACK (gtk_window_destroy), NULL);
